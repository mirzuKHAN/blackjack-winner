...
def test2a():
    mdp = player.CounterexampleMDP()
    mdp.computeStates()
    algorithm = player.ValueIteration()
...
def test3a():
    ...
    mdp2 = player.BlackjackMDP(cardValues=[1, 5], multiplicity=2,
...
def test3aHidden():
    mdp = player.BlackjackMDP(cardValues=[1, 3, 5, 8, 10], multiplicity=3,
...
def test3b():
    mdp = player.peekingMDP()
    vi = player.ValueIteration()
    vi.solve(mdp)
...
def test4a():
    ...
    mdp.computeStates()
    rl = player.QLearningAlgorithm(mdp.actions, mdp.discount(),
                                   player.identityFeatureExtractor,
                                   0)
...
def test4aHidden():
    smallMDP = player.BlackjackMDP(cardValues=[1, 5], multiplicity=2, threshold=10, peekCost=1)
    mdp = smallMDP
    mdp.computeStates()
    rl = player.QLearningAlgorithm(mdp.actions, mdp.discount(),
                                   player.identityFeatureExtractor,
...
def test4c():
    mdp = player.BlackjackMDP(cardValues=[1, 5], multiplicity=2,
                              threshold=10, peekCost=1)
    mdp.computeStates()
    rl = player.QLearningAlgorithm(mdp.actions, mdp.discount(),
                                   player.blackjackFeatureExtractor,
                                   0)
    # We call this here so that the stepSize will be 1
    rl.numIters = 1
